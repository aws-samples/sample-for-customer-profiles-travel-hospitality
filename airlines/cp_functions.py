...
def createCalculatedAttribute(client,domainName,calcAttrName):
    try:
        json_data = getObjectDef(calcAttrName)
        calc_attr_def = {
            'DomainName': domainName,
            'CalculatedAttributeName': json_data['CalculatedAttributeName'],
            'DisplayName': json_data['DisplayName'],
            'Description': json_data['Description'],
            'AttributeDetails': json_data['AttributeDetails'],
        ...
        print(calc_attr_def)
        client.create_calculated_attribute_definition(**calc_attr_def)
        print(f"Created Calculated Attribute {calcAttrName} successfully")
    except Exception as e:
        print(f"Error while creating {calcAttrName} Object type mapping {e}")
        traceback.print_exc()
